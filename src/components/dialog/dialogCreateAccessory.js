...
import { PictureAsPdf, PictureInPictureAltSharp } from '@material-ui/icons';
...
const DialogCreateAccessory = ({ data, open, onClose }) => {
    ...
    const handleUpdate = () => {
        if (image) {
            const uploadTask = storage.ref().child(`web_admin/accessories/${image.name}`).put(image)
            ...
            uploadTask.on(
                ...
                () => {
                    storage.ref("web_admin/accessories").child(image.name).getDownloadURL().then(
                        url => {
                            setURL(url);
                            setProgress(0)
                        }
                                        ...
                                        <Card style={{ display: 'flex' }}>
                                            <CardContent style={{ flex: '1 0 auto' }}>
                                                <Grid container justify="center" alignItems="center">
                                                    <input type="file" style={{ display:"none" }} onChange={(e) => { handleChangeImage, handleUpdate }} />{" "}
                                            ...
                                            <CardMedia >
                                                {url ? <img style={{ width: '130', height: '130' }} src={url} className="App-logo" alt="logo" /> : <img style={{ width: '130', height: '130' }} src={url} className="App-logo" alt="logone" />}
