...
import {  GET_MANUFACTURER_BY_NAME_URL, GET_MANUFACTURER_LIST_URL } from "src/services/Config";
...
export const listManufacturer = (keySearch) =>
  (dispatch) => {
    ...
    try {
      if (keySearch == undefined || keySearch == '') {
        ...
        // const arData = [];
        Axios.get(
          GET_MANUFACTURER_BY_NAME_URL + keySearch
        ).then(respo => {
          // arData.push(respo.data)
          // console.log(arData);
          dispatch({ type: MANUFACTURER_LIST_SUCCESS, payload: respo.data });
        })
      }
      localStorage.setItem('keySearch', keySearch);
      console.log(data,"manufacaction");
    } catch (error) {
      const message =
